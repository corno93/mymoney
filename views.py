...
async def upload(request: Request, file: UploadFile = File(...)):
    ...
    df = pd.read_csv(StringIO(data.decode('utf-8')), header=None, delimiter=",", usecols=[0, 1, 2], names=["date", "amount", "description"], parse_dates=['date'], dayfirst=True)
    ...
    return templates.TemplateResponse("index.html", {"request": request, "calendar": html_calendar})
